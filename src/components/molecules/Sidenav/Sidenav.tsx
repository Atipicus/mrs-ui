...
export const Sidenav = React.forwardRef<HTMLDivElement, SidenavProps>(
  (
    ...
    const renderGroup = (
      group: { header: string; items: SidenavItem[] },
      groupIndex: number
    ) => {
      if (variant === 'slim') {
        ...
        return (
          <React.Fragment key={`group-${groupIndex}`}>
            ...
            {group.items.map((item, index) =>
              renderNavItem(item, groupIndex * 100 + index)
            )}
          </React.Fragment>
        );
      }
