...
describe('SyncDialog', () => {
  describe('Visibility', () => {
    it('does not render when open is false', () => {
      renderWithTheme(<SyncDialog open={false} onClose={jest.fn()} />);
      expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
    });

    it('renders dialog when open is true', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} />);
  ...
  describe('Idle state (default)', () => {
    it('shows all sync steps', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} status="idle" />);
    ...
    it('calls onClose when Close is clicked', () => {
      ...
      renderWithTheme(<SyncDialog open onClose={onClose} status="idle" />);
  ...
  describe('Syncing state', () => {
    it('shows syncing title', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} status="syncing" />);
  ...
  describe('Success state', () => {
    it('shows success title', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} status="success" />);
      expect(screen.getByText(/sync complete/i)).toBeInTheDocument();
    });

    it('shows commit hash when provided', () => {
      renderWithTheme(
        <SyncDialog open onClose={jest.fn()} status="success" commitHash="abc1234" />
      );
      expect(screen.getByText(/abc1234/)).toBeInTheDocument();
    });

    it('does not show commit section when commitHash is null', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} status="success" commitHash={null} />);
  ...
  describe('Error state', () => {
    ...
    it('shows Close button (not Done) when status is error', () => {
      renderWithTheme(<SyncDialog open onClose={jest.fn()} status="error" />);
      expect(screen.getByRole('button', { name: /close/i })).toBeInTheDocument();
